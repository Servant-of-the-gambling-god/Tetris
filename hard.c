...
void StartGame() {
    int Total_block = 0, Total_change = 0, score_threshold = 150;
    usleep(1000);
    time_t current_time;
    struct tm *time_info;
    time(&current_time);
    ...
    int last_minute = current_minute;

    int shape = rand() % 7, form = rand() % 4;
    int change_shape = -1, change_form = -1;
    int record_space_block_Y;
    int difficult_layer = 0;
    int joker_active = 0;

    while (1) {
        
        Total_block = (Total_block + 1) % 11;
        Total_change = (Total_block + 1) % 5;
        time(&current_time);
        time_info = localtime(&current_time);
        current_minute = time_info->tm_min;
        if (current_minute != last_minute) {
            difficult_layer = PutHigher(difficult_layer);
        ...
        int Is_change = 0, nextShape, nextForm;
        ...
        joker_active = checkjoker();
        if (joker_active == 0) {
            joker_active = Put_JJ(joker_active);
        }
        ...
        while (1) {
            ...
            if (t == 0) {
                if (IsLegal(shape, form, x, y + 1) == 0) {
                    ...
                    if (check_block_now_put_on_BB(shape, form, x, y) == 1) {
                        if (difficult_layer > 0) difficult_layer--;
                    }
